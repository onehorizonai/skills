...
Handle operational requests over One Horizon work by choosing the right lookup, create, update, and comment path.

## One Horizon work model

- Initiative: roadmap-first planned work. Use for work that moves the product plan forward, usually spans multiple steps or days, and should roll up to goals, products, companies, components, or team reporting. Treat initiatives as reviewable specs for product work.
...
- Todo: simple personal follow-up. Use for small, private, owner-level tasks that do not need to show up as roadmap structure for the wider team. A completed implementation slice can also be written back as a completed Todo linked to an initiative.
...
- If the user describes a small personal reminder or one-off follow-up that only they need to see, default to a Todo.
- If implementation work was done in service of an initiative and the user wants write-back, a completed Todo linked to that initiative is an acceptable delivery note.
- If the user just says "create a task" without enough signal, classify in this order: initiative for roadmap work, bug for defects, ongoing for recurring operational work, Todo for private follow-up.
...
- The user wants help drafting or refining a structured initiative brief before creating roadmap work. Use `one-initiative-brief`.
...
- If multiple task matches are found, summarize the best matches and ask the user to pick before updating the wrong item.
...
- Do not create a Todo for work that should clearly live on the roadmap.
- Do not create an initiative for private follow-up or routine recurring work.
- If the request sounds like ongoing work but no direct ongoing-work create path is available, explain that distinction and ask one concise question before creating the wrong work type.

## Decision flow

### 1. Resolve scope

- If the user needs team or member IDs, use `one-find-team`.
- If the user needs to locate existing work by text, call `search-tasks`.
- If the user asks for active, completed, or blocked work rather than a specific item, use `one-list-work`.
...
- Personal follow-up or small owned work: Todo via `create-todo` or `update-todo`
...
1. Use `search-tasks`, `one-list-work`, or `one-find-team` to resolve the target.
...
- If the initiative is still fuzzy and the user needs help writing it well, use `one-initiative-brief` before creating it.
...
- When the workspace uses ongoing work, prefer that framing over roadmap initiative or private Todo.
